...
fig_scatter = figure(height=400, title="Opening Hours and Durations of Restaurants",
...
fig_kd = figure(height=400, x_axis_label="Hour of Opening", y_axis_label="Duration",
...
def compute_kernel_density_plots(attr, old, new, fig=fig_kd):
...
update_weekdays_callback = CustomJS(args=dict(source_weekdays=source_weekdays, weekdays=weekdays, fig=fig_kd), code="""
...
update_rating_groups_callback = CustomJS(args=dict(source_rating_groups=source_rating_groups, rating_groups=rating_groups, fig=fig_kd), code="""
    let active = cb_obj.active;  // Get the indices of checked boxes
...
dummy_glyphs = []
for rating_group in rating_groups:
    dummy_glyphs.append(fig_kd.scatter(1, 1, size=10, color=color_dict[rating_group], visible=False))
...
compute_kernel_density_plots(attr="active", old=[], new=[], fig=fig_kd)

# Put the plot and buttons in a layout and add to the document
curdoc().add_root(column(checkboxes_rating_groups, checkboxes_weekdays, btn_refresh, fig_scatter, fig_kd))