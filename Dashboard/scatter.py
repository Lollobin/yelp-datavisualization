...
def create_scatter_plot(df_business, weekdays):
    ...
    plot = figure(title="Opening Hours of Businesses",
    ...
    for i, day in enumerate(weekdays):
        ...
        plot.scatter(filtered_df[day + "_Hour_Of_Opening_Float"],
                     filtered_df[day + "_Open_Duration_Float"],
                     color=colors[i], size=7, alpha=0.3, legend_label=day)
...
def update_plot(attr, old, new, df_business, source, weekdays, min_hours, min_opening):
    ...
    colors = Colorblind[len(weekdays)]
    for i, day in enumerate(weekdays):
        filtered_df = df_business[(df_business[day + "_Open_Duration_Float"] >= min_hours) &
